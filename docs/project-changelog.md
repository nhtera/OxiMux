...
- **ACP terminal disabled-reason.** The view-options hint now distinguishes "send a message first" (genuinely no session) from "no interactive terminal for this agent" (a bound ACP chat) via a `TerminalAvailability` enum — fixing the GUI-found misleading hint. The interactive-resume **mechanism** (opencode `--session`, amp `threads continue`) is deferred to a follow-up: both are CLI-confirmed, but the ACP-session-id→CLI-id mapping and the concurrent-writer contract need a live GUI to verify.
