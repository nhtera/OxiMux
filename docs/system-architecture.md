...
`PiAdapter` is the third branded adapter: builds `pi [--session ID] [--model M] [<prompt>]` with the prompt as a trailing positional. It is also a chat backend of its own (`pi --mode rpc`) — the same dual terminal/chat nature Claude and Codex have. Empty `status_patterns()` with the same calibration deferral. (`CommandSpec::stdin_seed` remains a reserved path for adapters whose REPL takes no positional prompt; no built-in adapter currently uses it.)
