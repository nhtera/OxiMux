...
| `simulator` | `oximux-simulator` | `src/lib.rs` | gpui-free core of the iOS Simulator panel: the stdio protocol with `oximux-sim-helper` (`protocol.rs`, spec in the fork's `oximux/PROTOCOL.md`); spawning and supervising the helper (`helper.rs`, `session.rs`: latest-frame-wins stream, requests by id); `simctl`; availability gating (never runs `xcrun` without Xcode); a crash-safe child ledger; and pure geometry, keyboard, gesture, AX and command-classification code, plus the agent-control core (`consent.rs`: per-device answers, per-worktree requests; `agent.rs`: point ↔ touch mapping and input checks) behind `oximux sim …` (desktop side: `shell/simulator/agent_ops.rs`, reached through `remote_control/sim_bridge.rs`). The panel opens itself through one policy in `shell/simulator/auto_open.rs` (chat tool calls judged by `classify.rs`, a device booted where an agent works, `oximux sim` verbs; 60 s rate limit, "user closed it" memory); its settings pane is `shell/settings_modal/pane_simulator.rs` (`simulator.toml`, plus revoking the SQLite approvals through the hub). The helper is **not built here**: it is a prebuilt release of our serve-sim fork (`nhtera/serve-sim`, branch `oximux`), fetched by `scripts/fetch-sim-helper.sh` and checked against a pinned sha256. **Android** (P10) sits beside it: `android/` (SDK discovery, `adb`/`emulator` as data, AVDs as `avd:<name>` ids and phones as `adb:<serial>`, the scrcpy 4.1 control/video wire formats byte for byte, the pinned scrcpy server jar downloaded and sha256-checked on first use, `uiautomator` → the same AX tree, `session.rs` = a live device over adb-forwarded sockets) and `video/` (Annex-B → AVCC, the iOS helper's avcC records, VideoToolbox decode straight into the `core_video` buffers GPUI's `surface()` paints). The iOS helper streams H.264 too (protocol 2 `video` messages, decoded in `session.rs`, with a JPEG fallback); the stream row's Encoding menu (`simulator.toml` `stream.encoding`, default H.264) switches it live. `stream.rs`'s `StreamSession` is the platform-neutral session the hub, screen and agent verbs hold (desktop side: `shell/simulator/hub/android.rs`, `agent_ops/android.rs`) |
...
Every installed hook command names the **hook shim**, `~/.oximux/agent-hooks/oximux-agent-status`, never an OxiMux binary (`agent_hooks::hook_shim`). These files are global: every agent on the machine runs them, inside OxiMux or not. So a binary named there becomes the one all of those agents call, and the running binary may be a build output under cargo's `target/`. Such a binary cannot start outside cargo, and on a Mac with a stuck exit path each failed start left an unkillable process; 1490 piled up in two days. The shim is rewritten on every app start and on `oximux agent hooks on`. It runs the first executable of: the writing binary unless it is a build output, then `/Applications` and `~/Applications/OxiMux.app` (macOS), then the build output as a last resort (`OXIMUX_HOOKS_USE_DEV_BUILD=1` keeps it first). It always exits 0, draining the event when no OxiMux is installed. The per-spawn `--settings` JSON names the same shim, so the command strings stay byte-identical. On Windows, hooks still name the binary.
...
**Shell history across a restore.** A restored plain terminal's Up arrow reads the shell's own history file, so what it offers depends on that file being written. The zsh integration overlay (`shell/terminal/shell_integration.rs`) restores the user's `ZDOTDIR` before their `.zshrc`, points `HISTFILE` back at `<ZDOTDIR or $HOME>/.zsh_history` when macOS's `/etc/zshrc` derived it from the overlay dir, and sets `inc_append_history` (unless `share_history` / `inc_append_history_time` is on) so a shell killed with the relay or by a reboot keeps its commands. History and `.zcompdump*` caches that earlier versions stranded in the overlay dir are carried over / dropped at install.
