//! The desktop's [`TerminalSource`]: `remote-host`'s terminal seam implemented
//! over the relay daemon.
//!
...
///
/// A `OnceLock` rather than a return value threaded through `main` because the
/// relay comes up *before* the `RemoteControl` global exists, and the two are
/// installed in different scopes. This mirrors `install_shared_backend`, which
/// publishes the terminal backend from the same boot step for the same reason.
