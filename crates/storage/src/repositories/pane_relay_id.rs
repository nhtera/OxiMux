...
impl PaneRelayIdRepo {
    ...
    /// Upsert one pane's relay id at the given ordinal.
    pub fn set(
        &self,
        project_id: &str,
        ordinal: u32,
    ...
    /// Fetch every (ordinal, relay_pty_id, relay_session) for a project,
    /// ordered by `ordinal` ascending. Caller pairs the rows with leaves
    /// in the matching DFS order used by `pane_buffers` (V002).
    ...
    /// Delete every row for a project. Used by the reconciliation path
    /// to clear stale ids before writing a fresh snapshot — avoids
    /// leaving orphaned rows when the pane tree shrinks.
    pub fn delete_for_project(&self, project_id: &str) -> Result<(), StorageError> {
        self.db.with_conn(|c| {
            c.execute(
                "DELETE FROM pane_relay_ids WHERE project_id = ?1",
                params![project_id],
            )
            .map(|_| ())
        })?;
