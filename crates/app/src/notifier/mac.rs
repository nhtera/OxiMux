...
impl MacNotifier {
    ...
    /// receiver is a single GPUI-side task that drains promptly.
    pub fn new(click_tx: UnboundedSender<u64>) -> Self {
        Self {
            click_tx,
...
impl Notifier for MacNotifier {
    fn notify_needs_approval(&self, tab_id: TabId, agent_label: &str, reason: &str) {
        ...
        let title = format!("Agent needs approval — {agent_label}");
        let message = if reason.is_empty() {
            "Click to switch to the tab and respond.".to_string()
        } else {
            reason.to_string()
        };
