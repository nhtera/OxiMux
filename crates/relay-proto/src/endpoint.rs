...
mod tests {
    ...
    fn a_bumped_socket_version_derives_a_fresh_name() {
        ...
        // daemon. That only works if the name changes with it, so the two
        // versions here must stay *different* — comparing one against itself
        // would pass for a naming scheme that ignored the version entirely.
        let v8 = namespaced_name(&socket_path(&["home", "me", "app"], "relay-v8.sock"));
        let v9 = namespaced_name(&socket_path(&["home", "me", "app"], "relay-v9.sock"));
        assert_ne!(v8, v9);
    }
