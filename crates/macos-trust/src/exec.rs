...
//!
//! Every codesign/spctl/ditto call goes through here. The callers may sit on a
//! GUI main thread, so an unbounded wait on a wedged tool would freeze the
//! whole UI; the timeout converts that into a recoverable error.
...
/// that a fast command (the common case — codesign returns in well under a
/// second) is not padded by a whole poll interval.
...
pub fn run_bounded(program: &Path, args: &[&str], timeout: Duration) -> Result<Output, TrustError> {
    let mut child = Command::new(program)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|source| TrustError::Spawn {
...
mod tests {
    ...
    fn a_hung_child_times_out_instead_of_blocking() {
        ...
        assert!(matches!(err, TrustError::Timeout { .. }), "got {err:?}");
        assert!(
    ...
    fn missing_program_is_a_spawn_error() {
        let err = run_bounded(Path::new("/nonexistent/tool"), &[], Duration::from_secs(1))
            .expect_err("must fail");
        assert!(matches!(err, TrustError::Spawn { .. }), "got {err:?}");
    }
}
